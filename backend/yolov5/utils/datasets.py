...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        self.stride = stride
        self.auto = auto
        self.prev_A = None  # Store previous atmospheric light
        self.prev_t = None
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(img2label_paths([str(img)])[0]).exists():  # check label
            with open(path.parent / txt[i], 'a') as f:
                fst.write('./' + img.relative_to(path.parent).as_posix() + '\n')  # add image to txt file
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any([len(x) > 8 for x in lb]):  # is segment
                    classes = np.array([x[0] for x in lb], dtype=float)
                    segments = [np.array(x[1:], dtype=float).reshape(-1, 2) for x in lb]  # (cls, xy1...)
